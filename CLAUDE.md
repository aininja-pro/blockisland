...
**Completed (May 12, 2026) — CSV Export:**

33. **CSV export on Listings, Events, Categories pages** — "Export CSV" buttons download date-stamped CSVs (`block-island-{listings|events|categories}-YYYY-MM-DD.csv`). No new deps, no backend routes, no schema changes.
    - Shared util `admin/src/lib/csv.ts`: `toCsv(rows, columns)` (RFC-4180 escaping, `\r\n` lines, UTF-8 BOM for Excel), `downloadCsv(filename, content)` (Blob + anchor click), `todayStamp()`, `CsvColumn<T>` type.
    - **Listings**: button in the `DataTable` toolbar; exports `table.getFilteredRowModel().rows` so it respects active search + section/status/tier/renewal filters and includes all pages, not just the visible one. Columns: Name, Address, Status (Published/Draft), Tier (Premium/Basic), Section (distinct parent sections `; `-joined), Categories (distinct subcategories `; `-joined). Section/Categories resolved via `buildCategoryLookup`/`getAppearsIn` (now exported from `listings/columns.tsx`) from the `listing_categories` junction.
    - **Events**: button in the `EventDataTable` toolbar; same filtered-rows behavior. Columns: Event Name, Start Date, End Date (`YYYY-MM-DD HH:mm`, read with UTC getters per the wall-clock-as-UTC convention; all-day rows show date only), All Day (Yes/No), Status, Location, Category.
    - **Categories**: `ExportCategoriesButton` in the Sections page header (no filtering on that page — operates on the data the server component already fetched). One row per section followed by its subcategory rows. Columns: Name, Type (Section/Subcategory), Parent Section, Listings, Premium Listings, Subcategory Count (sections only).
    - The generic `csvExport?: { filename: () => string; columns: CsvColumn<TData>[] }` prop on both DataTables keeps the export logic where the filtered rows live. The pre-existing inline CSV export on the Listing Analytics page was left as-is (not refactored).
...
| Apr 15, 2026 | Imported-events time shift. Imported events (from March GoodBarber export) stored real UTC, while native admin events store wall-clock-labeled-as-UTC. Feed's `formatEasternWallClock` assumes the latter, so imported rows rendered 4h late (Bird Walk showed 12 PM instead of 8 AM). One-shot SQL migration `src/db/migration-fix-imported-event-times.sql` applies `AT TIME ZONE 'America/New_York' AT TIME ZONE 'UTC'` to future imported, non-all-day rows (`goodbarber_id IS NOT NULL AND start_date >= CURRENT_DATE AND all_day = false`). DST-aware: EDT rows shift -4h, EST rows -5h. | Monitor Rob's confirmation in the live app | Migration is single-run only — double-shifts if re-executed. Verified via feed: Bird Walk now `2026-04-21T08:00:00-04:00`. |
| Apr 15, 2026 | Event title cleanup. Confirmed GoodBarber renders "From X to Y" in gray on the event detail view automatically from the `date`/`endDate` ISO fields — so the verbose `— 8:00 AM – 9:00 AM ET` suffix on the title is redundant. Removed the suffix in `transformEventToGoodBarber`. Left the bold ET time line in content HTML as a belt-and-suspenders fallback for devices outside ET. Kept `subtype: "mcms"` (the working value for maps + events). | Monitor Rob's confirmation | Dead ends that didn't work: (a) emitting `date`/`endDate` as Unix epoch seconds — broke the feed entirely, reverted; (b) switching `subtype` to `"custom"` per GoodBarber's official `contentapi_custom_feeds_example/events.json` — no visible change, reverted. The official example doesn't reveal what triggers the gray time label; `mcms` + ISO strings is the working combo. GoodBarber's export JSON (the March dump) is lossy — stores `date: "T00:00:00"` with duration encoded via `endDate`, so don't look to it as a spec reference. |

| May 12, 2026 | CSV export on Listings, Events, Categories pages. Shared `admin/src/lib/csv.ts` util (RFC-4180 + UTF-8 BOM, no deps). Listings/Events buttons live in the `DataTable` toolbar and export the filtered row model (respects search/filters, all pages); Categories button in the page header. Generic `csvExport` prop on both DataTables. Verified `toCsv` output with standalone Node scripts; `npm run lint` clean. | Rob to test in the live admin; then Events/Categories UI click-through once dev server works | Could not run `next dev` to UI-test — it hangs at 0% CPU at startup in both Turbopack and webpack mode (Next 16.1.6 on Node v25.5.0); needs Node 22 LTS locally. The blueprint's file paths (`src/app/(admin)/...`) were stale — admin app is at `admin/src/app/(protected)/...`. CLAUDE.md tech stack line ("Next 15 + React 18") is also stale; repo is on Next 16.1.6 + React 19. |
