...
| Apr 12, 2026 | Event feed timezone fix: rewrite stored `+00:00` offset to America/New_York (DST-aware) in `events-feed.js:formatEasternWallClock`. 5 PM entered in admin was rendering as 1 PM on EDT phones and 12 PM on CDT phones. Deployed to Render via push to `client`. | Monitor Austin's confirmation | GoodBarber always converts event `date`/`endDate` to phone's local TZ — can't force ET display from feed side; would need text-in-title to override. Verified via live feed: `2026-04-25T17:00:00-04:00`, and CDT tester saw 4-5 PM (math checks out for EDT viewers seeing 5-6 PM). |
